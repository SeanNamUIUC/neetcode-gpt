...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.head_size = (attention_dim // num_heads) #to make it int
        self.single_heads = nn.ModuleList([])
        for i in range(num_heads):
            self.single_heads.append(self.SingleHeadAttention(embedding_dim, self.head_size))
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        res = []
        for single_head in self.single_heads:
            sub_res = single_head(embedded)
            res.append(sub_res)
        concat = torch.cat(res, dim = 2)
